			...
			for(var i=0; i<2; i++){
				...
				if(dp_curr){
					...
					if(parseFloat(dp_curr) > parseFloat(dp_yest)) {
						...
						$('#dpCurr_'+i).css({ color: "#00b700",'font-weight':"bold"});
					}
				}
				...
				if(parseFloat(dp_diff) > 0) {
					...
					$('#dpDiff_'+i).html("\跌 <font color='#00b700'\>" + dp_diff.toFixed(2) + "\<\/font\>");
					$('#dbPoint_'+i).html("\<font color='#00b700'\>" + ((dp_diff / dp_yest) * 100).toFixed(2) + "％\<\/font\>");
				} else {
...
function showgain()
{
	...
	// {
		...
		$.ajax({
			url: '/site/getdeal?type=getdeal&rand='+Math.random(),
			type: 'GET',
			dataType: 'html',
			success: function(response){
				//alert(response);
				response_arr = JSON.parse(response);
				var lcmoney=0; //留仓总金额  Total amount of positions
				var lcgain=0; //留仓总盈亏   Total profit and loss
				var total_save_money = 0; //总留仓费    Total retention fee
				//alert(String(response_arr));
				var bulls =String(response_arr).split(',');
				
				for(var i=1; i<bulls.length + 1; i++){
					var bus =bulls;
					//var bus_code = bus.split(',')[0];    //代码        Code
					var sell_price = bus[i-1];    //当前价格  Current price
					/*var inf=($('#inf_'+i).attr('data')).split(',');*/
					
					var inf=($('#inf_'+i).attr('value')).split(',');
					
					var bus_price =inf[0]; //买入价格    Buy price
					var bus_type=inf[1]; //升/跌   		 Up/down
					var bus_num=inf[2]; //买入数量 		  Buy quantity
					var bull_cost_money=inf[3]; //买入手续费   Buying fee
					var sell_cost=inf[4]; //卖出手续费率       Selling commission rate
					var save_money=inf[5]; //留仓费            Position fee					
					var dc_money = inf[6]; //点差费金额        Spread amount
					var bull_money=(bus_price*bus_num*100).toFixed(2); //买入金额         Purchase amount
					var sell_money=(sell_price*bus_num*100).toFixed(2); //卖出金额        Selling amount
					var sell_cost_money=(sell_money*sell_cost).toFixed(2); //卖出手续费   Selling fee 
					//总手续费 (买入手续费+卖出手续费+留仓费+点差费)         Total handling fee (buy handling fee + selling fee + holding fee + spread)
					//var start_time = 
					
					var all_cost_money=parseFloat(bull_cost_money)+parseFloat(sell_cost_money)+parseFloat(save_money)+parseFloat(dc_money);
					if(bus_type==1)
					{
						//浮盈(卖出金额-买入金额-手续费-留仓费)  부동 이익 (판매 금액 - 구매 금액 - 처리 수수료 - 보유 비용)
						...
						//浮盈(买入金额-卖出金额-买入手续费) 변동 이윤 (매수 금액 - 매도 금액 - 매입 수수료)
					...
					{
						$('#cur_price_'+i).html('现价：' + sell_price);
						
						$('#gain_'+i).html('盈亏：' +gain);
						if(parseFloat(gain) > 0) {
							$('#cur_price_'+i).css({ color: "#ff0011"});
							$('#gain_'+i).css({ color: "#ff0011"});
						} else if(parseFloat(gain) < 0) {
							$('#cur_price_'+i).css({ color: "#00b700"});
							$('#gain_'+i).css({ color: "#00b700"});
						}else {
							$('#cur_price_'+i).css({ color: "#000000"});
							$('#gain_'+i).css({ color: "#00b700"});
						}
					}else{
				...
				{
					$('#span_gain').css('color','#00b700');
				}
				$('#brsy').html('￥'+totalgain);
				$('#brjl').html('￥'+totalgain);
				$('#lcf').html('￥' + parseFloat(total_save_money).toFixed(2));
				$('#brye').html('￥'+totalgain);
				$('#totalgain').html(totalgain);				
...
/*
*获得列表的股票即时指数  Get a list of stock real-time indices
*/
function stockRe(){
				...
				for(var i=0; i<stocks.length-1; i++){
					...
					if(parseFloat(curr_f)>0){
						...
						if(parseFloat(curr_f) > parseFloat(yest_f)) {
							...
							$('#buyID_'+i).css({ color: "#00b700",'font-weight':"bold",'cursor':"pointer",'height':"100%",'WIDTH':"100%"});
						}else {
						...
						if(parseFloat(sell) > parseFloat(yest_f)) {
							...
							$('#cc'+i).css({ color: "#00b700",'font-weight':"bold",'height':"100%",'WIDTH':"100%"});
						}
						$('#cc'+i).html(sell);
						$('#d'+i).html(tday_f);
						if(parseFloat(temp_f) > 0) {
							...
							$('#f'+i).html("\<font color='#00b700'\>" + temp_f.toFixed(2) + "\<\/font\>");
							$('#g'+i).html("\<font color='#00b700'\>" + ((temp_f / yest_f) * 100).toFixed(2) + "%\<\/font\>");
						} else {
...
function sale_buy(code,name,num,id,limit_time,limit_cost)
{
	...
	// {
	// 	dourl += "&cansell=false";
	// 	if(confirm('系统提示：该股票买入时间不足' + limit_time + '分钟将加收' + limit_cost*100 + '%的手续费!确定卖出吗？'))    ///주식 매입 시간이 충분하지 않습니다.분에는 수수료가 부과됩니다! 판매 하시겠습니까?
	// 	{
	// 		ymPrompt.win({message:dourl,width:350,height:350,title:'平仓股票' + name + '[' + code + ']',iframe:true,handler:function(){self.location.href=self.location.href+'?rnd='+Math.random();}})
	// 	}
	// }
	// else
	// {
